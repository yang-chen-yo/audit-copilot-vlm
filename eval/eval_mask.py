# coding=utf-8
"""
eval_mask.py — 口罩稽查數量層級召回率評估

GT 對應：
  with_mask            → face mask（系統應偵測到）
  without_mask         → 不計入 face mask GT
  mask_weared_incorrect→ 不計入 face mask GT

執行方式：
  python eval_mask.py --template mask_inspection --n_images 10
"""
...
# ── Flags ─────────────────────────────────────────────────────────────────────
_TEMPLATE  = flags.DEFINE_string ('template',         'mask_inspection', '模板名稱')
...
# dataset GT 類別 → 評估類別
# with_mask → face mask（有戴，計入 GT）
# without_mask / mask_weared_incorrect → 不計入 face mask GT
LABEL_MAP = {
    'with_mask'            : 'face mask',
    'without_mask'         : None,
    'mask_weared_incorrect': None,
}
EVAL_CLASSES = ['face mask']

DATASET_DIR = Path.home() / '.cache/kagglehub/datasets/andrewmvd/face-mask-detection/versions/1'
...
def main(argv):
    ...
    print(f"{'圖片':<32} {'GT with_mask':>12} {'偵測 face mask':>14}")
    print("-" * 62)
    ...
    for xml_path in tqdm.tqdm(xml_files, desc='推論中', leave=False):
        ...
        print(f"{filename:<32} "
              f"{gt_counts.get('face mask', 0):>12} "
              f"{dt_counts.get('face mask', 0):>14}")

    # 4. 總計與召回率
    gt     = total_gt['face mask']
    det    = total_det['face mask']
    recall = det / gt if gt > 0 else 0.0

    print("-" * 62)
    print(f"{'總計':<32} {gt:>12} {det:>14}")
    print("\n" + "=" * 45)
    print(f"{'類別':<20} {'GT':>6} {'偵測':>6} {'召回率':>8}")
    print("-" * 45)
    print(f"{'face mask':<20} {gt:>6} {det:>6} {recall:>7.1%}")
    print("=" * 45)
    print(f"\n評估圖片：{len(xml_files)} 張")
    print(f"備註：GT 僅計算 with_mask，without_mask 不列入。\n")
